...
for f in $(seq 1 $(ls $(echo ./${CSV_DIR}/${NUM_PROCESS}/*_files_coor.csv) | wc -l)); do ./src/detect_mammals_images_videos.py "$f"'_files_coor.csv' ${BBOXES_DIR}'/'${NUM_PROCESS} ${CSV_DIR}'/'${NUM_PROCESS} data/models/md_v4.1.0.pb; done
end=$(date +"%s")
